...
class AppointmentRequestReviewController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $requests = AppointmentRequest::with(['service', 'patient', 'preferredDentist.user', 'convertedAppointment'])
            ->when($status !== 'all', fn ($q) => $q->where('request_status', $status))
            ->latest('request_id')
            ->paginate(15);

        return view('staff.appointment-requests.index', compact('requests', 'status'));
    }
    ...
    public function show(int $requestId): View
    {
        ...
        return view('staff.appointment-requests.show', compact('appointmentRequest', 'dentists'));
    }
    ...
    public function confirm(ConfirmAppointmentRequest $request, int $requestId): RedirectResponse
    {
        ...
        try {
            $appointment = $this->reviewService->confirm(
                ...
                dentistId: (int) $request->dentist_id,
            ...
            return back()->withErrors(['confirm' => $e->getMessage()])->withInput();
        }
    }
    ...
    public function reject(RejectAppointmentRequest $request, int $requestId): RedirectResponse
    {
        ...
        try {
            ...
            return back()->withErrors(['reject' => $e->getMessage()])->withInput();
        }
    }
    ...
    public function reschedule(RescheduleAppointmentRequest $request, int $requestId): RedirectResponse
    {
        ...
        try {
            ...
            return back()->withErrors(['reschedule' => $e->getMessage()])->withInput();
        }
    }
}
